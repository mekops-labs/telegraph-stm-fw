...
static void test_encode_empty_payload(void) {
    ...
    TEST_ASSERT_EQUAL_INT(
        IPC_FRAME_OVERHEAD,
        ipc_encode(buf, sizeof(buf), IPC_OP_GET_STATE, 1, NULL, 0));
}
...
static void test_encode_ack_and_nack(void) {
    ...
    TEST_ASSERT_EQUAL_INT(
        1 + IPC_FRAME_OVERHEAD,
        ipc_encode_nack(buf, sizeof(buf), 0x1234, IPC_ERR_BAD_OPCODE));
...
static void test_parse_two_frames_in_one_block(void) {
    ...
    b = ipc_encode(&buf[a], sizeof(buf) - a, IPC_OP_ACK, 0x0007, "\x05", 1);
...
static void test_parse_recovers_after_false_sof(void) {
    ...
    good =
        ipc_encode(&buf[bad], sizeof(buf) - bad, IPC_OP_STATE, 0x0012, "ok", 2);
...
static void test_parse_stalls_on_a_possible_length(void) {
    const uint8_t noise[5] = {IPC_SOF, IPC_SOF, IPC_SOF, IPC_SOF, IPC_SOF};
    uint8_t buf[32];
    int n;
...
static void test_push_rejects_bad_arguments(void) {
    ...
    TEST_ASSERT_EQUAL_UINT(
        0, ipc_parser_push(NULL, buf, sizeof(buf), on_frame, &g_cap));
    TEST_ASSERT_EQUAL_UINT(
        0, ipc_parser_push(&g_parser, NULL, 4, on_frame, &g_cap));
}
