...
static const char *g_weekday[] = {"Nie", "Pon", "Wto", "Śro",
                                  "Czw", "Pią", "Sob"};
    ...
    for (row = 0; row < a->h; row++) {
        for (col = 0; col < a->w; col++) {
            ...
            if (sx >= a->srcw || sy >= a->srch) {
                ...
                on = (a->src[(sy * stride) + (sx / 8)] & (0x80 >> (sx % 8))) !=
                     0;
            }
        ...
        if (maxlines > 1) {
            ...
            if (n > 1) {
...

                {
                    ...
                    for (k = 0; k < n; k++) {
                        xoffs[k] =
                            text_column(dev, &s[starts[k]], lens[k], align);
                    }
...
int hazk03_display_animate(int panel, int x, int y, int w, int h, bool vertical,
    ...
    uint8_t *buf =
        (panel == HAZK03_PANEL_SUB) ? g_anim_src_sub : g_anim_src_main;
...
int16_t hazk03_display_temperature(void) { return g_temp; }
